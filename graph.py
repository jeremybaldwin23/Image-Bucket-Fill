...
On my/our honor, <Jeremy Baldwin> and <Dillon Alpert>, this
...
class ImageGraph:
    ...
    def bfs(self, start_index, color):
        ...
        while not queue.is_empty():
            ...
            current = self.vertices[current_index]
            ...
            if not current.visited:
                ...
                for neighbor_index in current.edges:
                    ...
                    if not neighbor.visited and neighbor.color == current.prev_color:
...
def create_graph(data):
    ...
    size = int(data_lines[0].strip())
    v_num = int(data_lines[1].strip())
    vertices = []
    for i in range(2, 2 + v_num):
        x, y, color = data_lines[i].strip().split(",")
        x, y = int(x), int(y)
    ...
    start_edge = 2+v_num
    num_edges = int(data_lines[start_edge].strip())
    ...
    for i in range(start_edge + 1, start_edge + 1 + num_edges):
        from_index, to_index = data_lines[i].strip().split(",")
    ...
    bfill = data_lines[start_edge+ 1+ num_edges].strip().split(",")
    start_index = int(bfill[0])
    color = bfill[1]
